...
public class DataManagerExceptionFilter : AbpExceptionFilter
{
    ...
    private WrapResult<object> SimplifyMessage(ExceptionContext context)
    {
        ...
        var localizer = context.GetRequiredService<IStringLocalizer<DataManagerLocalizationResource>>();
        switch (context.Exception)
        {
            case AbpAuthorizationException:
                result.SetFail(localizer["MMS.DataManager:PermissionDenied"], "401");
                break;
            case AbpValidationException validationException:
                var errorMessage = localizer["MMS.DataManager:ParameterValidationFailed"] + ";" + validationException.ValidationErrors.JoinAsString(";");
                result.SetFail(errorMessage, "400");
                break;
            case EntityNotFoundException:
                result.SetFail(localizer["MMS.DataManager:EntityNotFound"], "506");
                break;
            case NotImplementedException:
                result.SetFail(localizer["MMS.DataManager:Unimplemented"], "507");
                break;
            case DbUpdateConcurrencyException:
                result.SetFail(localizer["MMS.DataManager:DbUpdateConcurrency"], "508");
                break;
            default:
            {
                if (context.Exception is IHasErrorCode codeException)
                {
                    var exceptionConverter = context.GetRequiredService<IDataManagerExceptionConverter>();
